...
def home(request):
    ...
    # 데이터베이스의 Post로부터 date 기준으로 전체 테이블을 가져옴
    posts = Post.objects.filter().order_by('-date')
    return render(request, 'index.html', {'posts':posts})


def postcreate(request):
    # POST나 FILES method라면 익명게시판 게시글을 저장하고 home으로 이동시킴
    if request.method == 'POST' or request.method == 'FILES':
        form = PostForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            return redirect('home')
    # 아니라면 get(url을 타고 들어오는 PostForm을 return 시켜주어 Form을 화면에 보여줌)
    else:
        form = PostForm()
...
def detail(request, post_id):
    # <id>로 들어오는 request에 맞추어 해당하는 post_detail 화면을 보여줌
    post_detail = get_object_or_404(Post, pk=post_id)
    # <id>로 들어오는 게시글에 해당하는 댓글을 달 수 있도록 CommentForm을 띄워줌
